...
app.post('/api/config', async (req, res) => {
  const { openThreshold, wateringMinutes, resetWifi } = req.body;
  ...
  if (resetWifi === false)           config.resetWifi       = false;

  console.log(`[CONFIG] Open <${config.openThreshold}% | Water ${config.wateringMinutes} min | resetWifi: ${config.resetWifi}`);
  await saveConfig();
